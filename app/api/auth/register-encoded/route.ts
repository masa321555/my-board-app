...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Base64デコード
    const password = body.passwordEncoded ? atob(body.passwordEncoded) : body.password;
    const confirmPassword = body.confirmPasswordEncoded ? atob(body.confirmPasswordEncoded) : body.confirmPassword;
    ...
    console.log('エンコード版登録リクエスト受信:', {
      name: decodedData.name,
      email: decodedData.email,
      hasPassword: !!decodedData.password,
      passwordLength: decodedData.password?.length,
    });
    
    // バリデーション
    if (!decodedData.name || !decodedData.email || !decodedData.password || !decodedData.confirmPassword) {
      return errorResponse('必須フィールドが不足しています', 'MISSING_FIELDS', 400);
    }
    
    if (decodedData.password !== decodedData.confirmPassword) {
      return errorResponse('パスワードが一致しません', 'PASSWORD_MISMATCH', 400);
    }
    
    if (decodedData.password.length < 8) {
    ...
    // パスワード強度チェック（本番環境のみ）
    if (!isDevelopment) {
      const passwordStrength = checkPasswordStrength(decodedData.password);
    ...
    // パスワードをハッシュ化
    const hashedPassword = await bcrypt.hash(decodedData.password, 12);
    ...
    console.error('エンコード版登録エラー:', error);
